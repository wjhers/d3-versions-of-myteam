"""
This file implements the replay buffer. 
It is based on the replay buffer from stable-baselines, with some changes.
"""
import random
import numpy as np
...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        acts_n: np.array
            batch of actions executed given obs_n
        rew_n: np.array
            rewards received as results of executing acts_n
        next_obs_n: np.array
            next set of observations seen after executing acts_n
        done_n: np.array
            done_n[i] = 1 if executing acts_n[i] resulted in
        ...
        obs_n, acts_n, rew_n, next_obs_n, done_n = self.sample_index(idxes)
...
class EfficientReplayBuffer(object):
    def __init__(self, size, n_agents, obs_shape_n, act_shape_n):
        ...
        for idx in range(n_agents): # np.empty() to generate a shape array
