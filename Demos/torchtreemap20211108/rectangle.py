...
class Rect(object):
    ...
    def toMove(self, action_n_i):
        if action_n_i == 0: # up
            self._y -= 1
        elif action_n_i == 1: # down
            self._y += 1
        elif action_n_i == 2: # left
            self._x -= 1
        elif action_n_i == 3: # right
            self._x += 1
        elif action_n_i == 4: # up-left
            self._x -= 1
            self._y -= 1
        elif action_n_i == 5: # up-right
            self._x += 1
            self._y -= 1
        elif action_n_i == 6: # down-left
            self._x -= 1
            self._y += 1
        elif action_n_i == 7: # down-right
            self._x += 1
            self._y += 1
        elif action_n_i == 8: # w+
            self._width_half += 1
            self._height_half = round((self.getArea()/self.getWidth())/2)
            # 如果width增大很多，导致height为0
            if self._height_half == 0:
                self._height_half += 1
        elif action_n_i == 9: # w-
            self._width_half -= 1
            # 如果width减小到0后，给width加1
            if self._width_half == 0:
                self._width_half += 1
            self._height_half = round((self.getArea()/self.getWidth())/2)
